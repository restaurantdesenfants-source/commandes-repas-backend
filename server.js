...
// Vacances scolaires Wallonie 2026-2027 (mêmes dates que public/shared.js — dupliqué
// ici car ce fichier tourne côté serveur, sans accès au navigateur).
const HOLIDAYS_2026_2027 = [
  { label: "Toussaint", start: "2026-10-19", end: "2026-11-01" },
  { label: "Noël", start: "2026-12-21", end: "2027-01-03" },
  { label: "Carnaval", start: "2027-02-22", end: "2027-03-07" },
  { label: "Printemps", start: "2027-04-26", end: "2027-05-09" },
];
...
function holidayFor(date) {
  ...
  return HOLIDAYS_2026_2027.find((h) => iso >= h.start && iso <= h.end) || null;
}
...
app.post("/api/orders", async (req, res) => {
  try {
    ...
    // Sécurité supplémentaire : aucun repas n'est jamais enregistré un jour de
    // vacances scolaires, même si la requête a contourné l'interface.
    JOURS.forEach((j) => {
      if (holidayFor(dateForDay(weekKey, j.id))) {
        weekWithDessert[j.id] = { soupe: 0, maternelle: 0, primaire: 0, primairePlus: 0, dessert: 0 };
      }
    });
...
// Applique des modifications de quantités (partielles ou complètes) pour une école
// et une semaine données. Si aucune commande n'existe encore pour cette école cette
// semaine-là (ex. école qui a oublié de commander), elle est créée à la volée à
// partir de l'annuaire des écoles. weekUpdates peut ne contenir que certains jours :
...
// par une école, ou encodage d'une commande oubliée). Si l'école n'a rien commandé
// cette semaine-là, la commande est créée. Chaque changement est journalisé comme
// une rectification, pour apparaître dans l'historique de facturation et l'alerte
// "⚠️ Attention" côté cuisine.
...
// Encodage en masse pour une école sur plusieurs semaines d'un coup (proposition
// d'encodage pour un mois entier). "days" est une liste de jours à encoder, chacun
// rattaché à sa semaine (weekKey) ; un jour absent de la liste (retiré par la
// cuisine avant validation) n'est pas touché.
app.post("/api/orders/admin-bulk-month", async (req, res) => {
  try {
    const { code, schoolName, days } = req.body || {};
    if (!process.env.KITCHEN_CODE || code !== process.env.KITCHEN_CODE) {
      return res.status(401).json({ ok: false, error: "Code cuisine incorrect." });
    }
    if (!schoolName || !Array.isArray(days) || !days.length) {
      return res.status(400).json({ ok: false, error: "Informations manquantes." });
    }
    const byWeek = {};
    for (const d of days) {
      if (!d || !d.weekKey || !d.dayId) continue;
      if (holidayFor(dateForDay(d.weekKey, d.dayId))) continue; // sécurité : jamais un jour de vacances
      if (!byWeek[d.weekKey]) byWeek[d.weekKey] = {};
      byWeek[d.weekKey][d.dayId] = {
        soupe: Number(d.soupe || 0),
        maternelle: Number(d.maternelle || 0),
        primaire: Number(d.primaire || 0),
        primairePlus: Number(d.primairePlus || 0),
        dessert: Number(d.dessert || 0),
      };
    }
...
app.get("/api/orders", async (req, res) => {
  try {
    ...
    const corrections = correctionRows
      .map((c) => {
        const newValues = c.new_values || {};
        const { __source, ...values } = newValues;
        return {
          id: c.id,
          ...
          delta: c.delta,
          newValues: values,
          source: __source || "cuisine", // anciennes entrées sans source connue = cuisine par défaut
        };
      })
